...
from hypreturncodes import *



class Docker(HypBase):
  ...
  def _get_client(self, host):
    return docker.Client(base_url="http://"+host+":"+self.__port, version=self.__version)
  ...
  def deploy(self, host, user, vnf_type, vnf_name):
    ...
    with self._error_handling(return_data):
      dcx = self._get_client(host)
      name = user+"-"+vnf_name
      container = dcx.create_container(image=self.__image_prefix+'-'+vnf_type, hostname=name, name=name)
  ...
  def start(self, host, vnf_id):
    ...
    with self._error_handling(return_data):
      dcx = self._get_client(host)
      response = dcx.start(container=vnf_id, dns=self.__dns_list, privileged=True)
  ...
  def execute_in_guest(self, host, vnf_id, cmd):
      ...
      with self._error_handling(return_data):
        dcx = self._get_client(host)
        response = dcx.execute(vnf_id, ["/bin/bash", "-c", cmd], stdout=True,
                               stderr=False); 
        #return response, return_data['code'], return_data['message']
        return response
