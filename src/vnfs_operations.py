...
class VNFSOperations:
    ...
    def __init__(self, vnfs_root):
        ...
        self._hypervisor = vnfs_docker.Docker()
    ...
    def vnfs_create_vnf_instance(self, path, mode):
        ...
        os.open(full_path + "/status", os.O_WRONLY | os.O_CREAT, default_file_mode)
    ...
    def vnfs_is_nf_instance(self, path):
        tokens = self._full_path(path).encode('ascii').split('/')
        if ('nf-types' in tokens) and len(tokens) > tokens.index('nf-types') + 1:
            return True
        return False
    ...
    def vnfs_deploy_nf_chain(self, chain_root):
        ...
        ret_codes = [{nf_path: self.vnfs_deploy_nf(nf_path)} for nf_path in nf_paths]
        return ret_codes
    ...
    def vnfs_get_rx_bytes(self, nf_path):
        ...
        response = self._hypervisor.execute_in_guest(ip_address, cont_id, command)
        return response
    ...
    def vnfs_get_status(self, nf_path):
        ...
        cont_id, ret_code = self._hypervisor.get_id(ip_address,
                                                    getpass.getuser(),
                                                    nf_type, nf_instance_name)
        response, ret_code, ret_message = self._hypervisor.guest_status(ip_address, cont_id)
        return response
